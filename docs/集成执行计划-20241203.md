# 🚀 AlphaCouncil 智能投资系统集成执行计划
> 日期：2024-12-03 17:10
> 版本：v2.0.0 - 多智能体博弈增强版

## 📊 一、项目现状总结

### 1.1 已完成部分
✅ **基础架构**
- 10个智能体的线性分析流程
- Vue3前端框架（alpha-council-vue）
- FastAPI后端服务
- 基础股票数据接口

✅ **已迁移代码**
- agents/目录下的分析师、研究员、风控、交易员代码
- backend/dataflows/部分数据流代码
- 基础的提示词和文档

### 1.2 待解决问题
❌ **导入路径问题**
- 所有代码仍使用`from tradingagents.xxx import`
- 需要统一修改为本项目路径

❌ **功能未集成**
- 新闻舆情分析未接入API
- 多空辩论机制未激活
- 模拟交易系统需要完善
- 闭环验证机制缺失

## 🎯 二、分阶段执行计划

### 第一阶段：路径修复和基础集成（今天完成）

#### 1.1 创建路径映射工具
```python
# scripts/fix_imports.py
# 批量修复所有导入路径
```

#### 1.2 修复所有导入语句
- 将`tradingagents.` → 改为项目本地路径
- 创建统一的日志系统
- 修复工具类导入

#### 1.3 测试基础功能
- 验证所有智能体可以正常创建
- 测试数据接口是否正常

### 第二阶段：新闻舆情集成（第1-2天）

#### 2.1 集成新闻数据源
- 聚合数据API（已有KEY）
- 东方财富网爬虫
- 雪球网舆情爬虫
- 财联社快讯接口

#### 2.2 创建新闻API接口
```python
# backend/api/news_api.py
@router.post("/api/news/analyze")
async def analyze_news(stock_code: str):
    # 新闻情绪分析
    pass
```

#### 2.3 前端集成
- 在Vue中添加新闻分析组件
- 实时展示新闻情绪指标

### 第三阶段：多智能体博弈（第3-4天）

#### 3.1 激活多空辩论机制
- 集成bull_researcher和bear_researcher
- 实现research_manager决策整合

#### 3.2 风控三方辩论
- 激进、保守、中立三方博弈
- risk_manager最终裁决

#### 3.3 决策可视化
- 辩论过程可视化
- 决策路径展示

### 第四阶段：模拟交易增强（第5-6天）

#### 4.1 完善模拟交易系统
- 结构化决策输出
- 自动执行交易
- 持仓跟踪

#### 4.2 收益率跟踪
- 日收益曲线
- 策略对比分析
- 回测功能

#### 4.3 可视化面板
- 实时持仓展示
- 收益率图表
- 风险指标监控

### 第五阶段：闭环验证（第7天）

#### 5.1 决策验证系统
- 记录所有决策
- 跟踪实际表现
- 自动评分机制

#### 5.2 策略优化
- 基于历史表现调整权重
- 智能体表现评估
- 自适应参数调整

## 📋 三、立即执行任务

### 3.1 第一步：修复导入路径（现在开始）

创建路径修复脚本，批量替换所有导入语句：

```python
# scripts/fix_imports.py
import os
import re

def fix_imports_in_file(filepath):
    """修复单个文件的导入语句"""
    replacements = {
        'from tradingagents.agents': 'from agents',
        'from tradingagents.tools': 'from backend.tools',
        'from tradingagents.dataflows': 'from backend.dataflows',
        'from tradingagents.utils': 'from agents.utils',
        'import tradingagents': '# import tradingagents (removed)',
    }
    
    # 读取并替换
    with open(filepath, 'r', encoding='utf-8') as f:
        content = f.read()
    
    for old, new in replacements.items():
        content = re.sub(old, new, content)
    
    # 写回文件
    with open(filepath, 'w', encoding='utf-8') as f:
        f.write(content)
```

### 3.2 第二步：创建统一日志系统

```python
# backend/utils/logging_config.py
import logging
import os
from datetime import datetime

def get_logger(name):
    """获取统一配置的日志器"""
    logger = logging.getLogger(name)
    
    if not logger.handlers:
        # 创建日志目录
        log_dir = "logs"
        os.makedirs(log_dir, exist_ok=True)
        
        # 设置日志格式
        formatter = logging.Formatter(
            '%(asctime)s - %(name)s - %(levelname)s - %(message)s'
        )
        
        # 文件处理器
        file_handler = logging.FileHandler(
            f'{log_dir}/alpha_{datetime.now():%Y%m%d}.log',
            encoding='utf-8'
        )
        file_handler.setFormatter(formatter)
        
        # 控制台处理器
        console_handler = logging.StreamHandler()
        console_handler.setFormatter(formatter)
        
        # 添加处理器
        logger.addHandler(file_handler)
        logger.addHandler(console_handler)
        logger.setLevel(logging.INFO)
    
    return logger
```

### 3.3 第三步：创建中国市场数据爬虫

```python
# backend/dataflows/china_market_crawler.py
"""中国市场特定数据爬虫"""

import requests
from bs4 import BeautifulSoup
import akshare as ak
import tushare as ts
from backend.utils.logging_config import get_logger

logger = get_logger("china_market_crawler")

class ChinaMarketCrawler:
    """中国市场数据爬虫"""
    
    def __init__(self):
        self.tushare_token = os.getenv('TUSHARE_TOKEN')
        if self.tushare_token:
            ts.set_token(self.tushare_token)
            self.pro = ts.pro_api()
    
    def get_eastmoney_news(self, stock_code):
        """获取东方财富网新闻"""
        # 实现东财新闻爬取
        pass
    
    def get_xueqiu_comments(self, stock_code):
        """获取雪球网评论舆情"""
        # 实现雪球爬虫
        pass
    
    def get_cls_news(self):
        """获取财联社快讯"""
        # 实现财联社爬虫
        pass
    
    def get_sina_news(self, stock_code):
        """获取新浪财经新闻"""
        # 实现新浪爬虫
        pass
```

## 📈 四、预期成果

### 短期（本周）
- ✅ 路径问题完全解决
- ✅ 新闻舆情功能上线
- ✅ 多空辩论机制激活
- ✅ 基础模拟交易运行

### 中期（2周）
- ✅ 完整的博弈决策系统
- ✅ 自动化模拟交易
- ✅ 可视化决策面板
- ✅ 初步闭环验证

### 长期（1个月）
- ✅ 策略自动优化
- ✅ 多策略对比回测
- ✅ 实盘对接准备
- ✅ 用户持仓定制评测

## 🔧 五、技术架构

```
AlphaCouncil/
├── agents/              # 智能体核心（15个）
│   ├── analysts/        # 8个分析师
│   ├── researchers/     # 多空辩论
│   ├── managers/        # 决策管理
│   ├── risk_mgmt/       # 风控辩论
│   └── trader/          # 交易执行
├── backend/
│   ├── api/            # FastAPI接口
│   ├── dataflows/      # 数据流管理
│   ├── tools/          # 工具类
│   └── utils/          # 工具函数
├── alpha-council-vue/   # Vue3前端
│   ├── src/
│   │   ├── views/      # 页面组件
│   │   ├── components/ # 通用组件
│   │   └── stores/     # Pinia状态
│   └── dist/           # 构建输出
└── docs/               # 文档
```

## 📝 六、关键配置

### 6.1 环境变量（.env）
```
# LLM配置
OPENAI_API_KEY=sk-xxx
OPENAI_BASE_URL=https://api.deepseek.com

# 数据源配置
TUSHARE_TOKEN=xxx
JUHE_API_KEY=xxx
AKSHARE_ENABLED=true

# 系统配置
DEBUG_MODE=true
LOG_LEVEL=INFO
```

### 6.2 数据源优先级
1. Tushare（付费，稳定）
2. AkShare（免费，备用）
3. 聚合数据（新闻）
4. 网页爬虫（降级）

## 🚦 七、执行状态跟踪

| 任务 | 状态 | 完成时间 | 负责人 |
|-----|------|---------|--------|
| 路径修复 | 🔄 进行中 | 今天 | AI |
| 日志系统 | ⏳ 待开始 | 今天 | AI |
| 新闻爬虫 | ⏳ 待开始 | 明天 | AI |
| 多空辩论 | ⏳ 待开始 | 第3天 | AI |
| 模拟交易 | ⏳ 待开始 | 第5天 | AI |
| 闭环验证 | ⏳ 待开始 | 第7天 | AI |

---

**立即开始执行！**
