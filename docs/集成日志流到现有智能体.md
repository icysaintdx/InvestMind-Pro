# 集成日志流到现有智能体

**目标**: 将实时日志流功能集成到 AlphaCouncil 的现有智能体中  
**创建时间**: 2025-12-08 03:26  

---

## 📋 当前状态

### ✅ 已完成
1. **后端 SSE API** - `backend/api/agent_logs_api.py`
2. **日志处理器** - `backend/utils/log_stream_handler.py`
3. **前端日志显示** - `AgentCard.vue` 已支持 `fetching` 状态
4. **测试示例** - `backend/examples/agent_with_log_stream.py`

### 🔄 当前流程
```
AnalysisView.vue (runAgentAnalysis)
  ↓
agentStatus.value[agent.id] = 'fetching'  ← 已有，会触发日志显示
  ↓
fetchNewsData() / fetchFundFlowData()  ← 需要添加日志流
  ↓
agentStatus.value[agent.id] = 'analyzing'  ← 切换到骨架屏
  ↓
调用 LLM API
  ↓
agentStatus.value[agent.id] = 'success'
```

---

## 🎯 集成方案

### 方案1: 修改现有 API 端点（推荐）⭐⭐⭐⭐⭐

在现有的数据获取 API 中添加日志推送。

#### 优点
- 无需修改前端代码
- 日志自动推送
- 与现有流程完美集成

#### 示例：修改统一新闻 API

**文件**: `backend/api/unified_news_api_endpoint.py`

```python
from backend.utils.log_stream_handler import attach_log_stream, detach_log_stream
from backend.api.agent_logs_api import end_agent_log_stream
import logging

logger = logging.getLogger(__name__)

@router.post("/stock")
async def get_stock_news(request: StockNewsRequest):
    """获取股票新闻（带日志流）"""
    stock_code = request.stock_code
    agent_id = request.agent_id or "news_analyst"  # 新增参数
    
    # 附加日志流处理器
    handler = attach_log_stream(__name__, agent_id)
    
    try:
        logger.info(f"收到股票新闻请求: {stock_code}")
        logger.info(f"开始获取{stock_code}的综合新闻数据...")
        
        # 原有的数据获取逻辑
        result = await unified_news_api.get_comprehensive_news(stock_code)
        
        # 数据获取过程中的日志会自动推送
        
        logger.info("✅ 综合新闻数据获取完成")
        
        # 结束日志流
        end_agent_log_stream(agent_id)
        
        return {"success": True, "data": result}
        
    except Exception as e:
        logger.error(f"❌ 获取新闻失败: {str(e)}")
        end_agent_log_stream(agent_id)
        raise
    finally:
        detach_log_stream(__name__, handler)
```

**前端调用**（无需修改）:
```javascript
// AnalysisView.vue - runAgentAnalysis()
agentStatus.value[agent.id] = 'fetching'  // 触发 SSE 连接

const newsResult = await fetchNewsData(data.symbol)  // 后端自动推送日志

agentStatus.value[agent.id] = 'analyzing'  // 断开 SSE，显示骨架屏
```

---

### 方案2: 前端主动建立连接（备选）⭐⭐⭐

前端在调用 API 前主动建立 SSE 连接。

#### 优点
- 后端无需修改
- 灵活控制

#### 缺点
- 需要修改前端代码
- 需要手动管理连接

#### 示例

```javascript
// AnalysisView.vue
const runAgentAnalysis = async (agent, data) => {
  agentStatus.value[agent.id] = 'fetching'
  
  // 前端主动建立 SSE 连接
  // AgentCard 会自动监听 status 变化并建立连接
  
  try {
    // 调用 API
    const newsResult = await fetchNewsData(data.symbol)
    
    // 切换状态
    agentStatus.value[agent.id] = 'analyzing'
    
    // ... LLM 调用 ...
    
  } catch (e) {
    console.error(e)
  }
}
```

---

## 🔧 具体实现步骤

### 步骤1: 修改统一新闻 API

<function_calls>
<invoke name="read_file">
<parameter name="file_path">d:\AlphaCouncil\backend\api\unified_news_api_endpoint.py
