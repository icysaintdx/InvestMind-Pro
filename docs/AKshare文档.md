...
基于 Python 的代码如下： pip install akshare -i http://mirrors.aliyun.com/pypi/simple/ --trusted-host=mirrors.aliyun.com  --upgrade

基于 Anaconda 的代码如下： pip install akshare -i http://mirrors.aliyun.com/pypi/simple/ --trusted-host=mirrors.aliyun.com  --user  --upgrade

使用代理安装，由于相关限制，在这里对代理的安装和使用不做介绍，请自行搜索配置。

运行任意接口发现报错，错误类似：AttributeError: module 'akshare' has no attribute 'xxx'

检查 Python 的版本需要在 Python 3.8 以上，推荐使用 Python 3.10.x 及以上版本

检查是否安装了最新版本的 AKShare，如果不是最新版本，请先升级至最新版

检查在文档中是否具有该 xxx 接口，特定情况下有可能会改变接口的命名或者移除某些接口

检查所运行的 Python 文件名命名问题，不能用如下命名：akshare.py 与本地安装包冲突

不能获取指定的日期期间的数据，比如从 20200401 至 20200415 的数据

由于目标网页的大部分的接口一次性返回所有数据，所以在 AKShare 的部分接口函数中没有设置类似 start_date 和 end_date 的参数

如果要获取指定日期间的数据，请在调用接口后自行处理

接口报错出现类似错误提示： ReadTimeout: HTTPConnectionPool(host="www.xxx.com")
...
接口报错出现类似错误提示：cannot import name 'StringIO' from 'pandas.compat'

建议安装 pandas 版本大于 0.25.2，建议 pandas 版本大于 1.0

升级命令如下: pip install pandas --upgrade

出现数据返回错位，如日期数据和价格数据返回错位的情况

多运行几次，查看是否是网络不稳定造成的问题
...
Linux 系统显示 execjs._exceptions.RuntimeUnavailableError: Could not find an available JavaScript runtime.
...
stock_zh_index_daily_df = ak.stock_zh_index_daily(symbol="sz399552")
print(stock_zh_index_daily_df)
出现 AttributeError: 'MiniRacer' object has no attribute 'ext' 报错
...
查询 raw.githubusercontent.com 的真实 IP 地址

找到系统 host

Windows 10 在目录 C:\Windows\System32\drivers\etc

Ubuntu 18.04 在目录 /etc/hosts

修改 host

添加如下内容: 199.232.28.133 raw.githubusercontent.com

此处 199.232.28.133 为查找到的真实 IP

使用代理的时候报错 check_hostname requires server_hostname

降低 urllib3 的版本到 1.25.8：pip install urllib3==1.25.8
...
AKShare 快速入门
查看数据
具体函数使用详情, 请查看 AKShare 文档 每个接口的示例代码
...
1.4.86	fund_em_aum	fund_aum_em
...
AKShare 依赖说明
Python 依赖
AKShare 文档的依赖说明部分主要是为了对 AKShare 库的所有依赖库做一个描述， 方便小伙伴在对 AKShare 进行二次封装进行参考；

提供选择该库函数的部分原因说明；

所有的依赖名称都跟 PyPI 提供的库名称统一；

打包的时候注意 mini-racer 库如报错，请重新编译。
...
由于 PyExecJS 在 20180118 推出最后一个版本后， 主要的开发者不再对该库进行升级维护，导致部分问题无法通过升级该库来修复， 该库的 GitHub 地址 可以访问如下地址，所以没有使用该库;

Js2Py 是目前比较使用量较大和维护较好的库，其 GitHub 地址 但是考虑到在测试中， 对部分 Javascript 代码的运行不稳定，所以没有使用该库。
...
该库主要用于采集后的数据清洗，此处建议升级到最新版，AKShare 会优先支持最新的版本；
...
AKShare HTTP 部署
说明
AKShare HTTP 版本的部署主要依赖 AKTools、AKShare、FastAPI、Uvicorn、Typer 等开源 Python 库，部署的核心程序在 AKTools 中。 非常感谢给 AKTools 项目 点 Star，您的支持是我们持续开发最大的动力！
...
python -m aktools
版本说明
仅体验 HTTP API 功能则只需要安装：pip install aktools==0.0.68 版本；

体验完整功能请安装最新版，支持用于认证、权限、可视化页面等更多功能。
