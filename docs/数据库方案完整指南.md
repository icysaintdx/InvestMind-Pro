# 数据库方案完整指南

## 🎉 已完成功能

完整的数据库持久化方案已实现！

### ✅ 核心功能

1. **数据持久化** - SQLite/PostgreSQL 存储
2. **历史记录** - 完整的分析历史查询
3. **统计分析** - 智能体性能、股票热度统计
4. **跨设备同步** - 基于数据库的状态同步

---

## 📊 数据库设计

### 表结构

#### 1. `analysis_sessions` - 分析会话表
```sql
- id: 主键
- session_id: 会话ID（唯一）
- stock_code: 股票代码
- stock_name: 股票名称
- status: 状态（created/running/completed/error）
- progress: 进度（0-100）
- current_stage: 当前阶段（1-4）
- start_time: 开始时间
- end_time: 结束时间
- error_message: 错误信息
- created_at: 创建时间
```

#### 2. `agent_results` - 智能体结果表
```sql
- id: 主键
- session_id: 会话ID（外键）
- agent_id: 智能体ID
- agent_name: 智能体名称
- status: 状态
- output: 输出结果
- tokens: Token数量
- thoughts: 思维链（JSON）
- data_sources: 数据源（JSON）
- start_time: 开始时间
- end_time: 结束时间
- duration_seconds: 耗时（秒）
- error_message: 错误信息
- created_at: 创建时间
```

#### 3. `stock_history` - 股票历史统计表
```sql
- id: 主键
- stock_code: 股票代码（唯一）
- stock_name: 股票名称
- analysis_count: 分析次数
- last_analysis_time: 最后分析时间
- avg_analysis_duration: 平均分析时长
- created_at: 创建时间
- updated_at: 更新时间
```

---

## 🚀 快速开始

### 1. 初始化数据库

```bash
cd d:\AlphaCouncil\backend
python init_database.py
```

**输出**：
```
============================================================
AlphaCouncil 数据库初始化
============================================================

1. 测试数据库连接...
✅ 数据库连接成功

2. 是否需要重建所有表？
   输入 'yes' 确认重建，其他键跳过: no

3. 创建数据库表...
✅ 数据库表创建完成

4. 数据库表结构:
   - analysis_sessions: 分析会话表
   - agent_results: 智能体结果表
   - stock_history: 股票历史统计表

============================================================
✅ 数据库初始化完成！
============================================================
```

### 2. 启动服务器

```bash
python server.py
```

### 3. 测试 API

```bash
# 查看活跃会话
curl http://localhost:8000/api/analysis/db/sessions/active

# 查看热门股票
curl http://localhost:8000/api/analysis/db/history/popular-stocks

# 查看统计信息
curl http://localhost:8000/api/analysis/db/stats/overview
```

---

## 📡 API 端点

### 会话管理

#### 创建会话
```http
POST /api/analysis/db/session/create
Content-Type: application/json

{
  "stock_code": "600000",
  "stock_name": "浦发银行"
}
```

#### 查询会话状态
```http
GET /api/analysis/db/session/{session_id}/status
```

#### 更新会话进度
```http
POST /api/analysis/db/session/{session_id}/update
Content-Type: application/json

{
  "agent_id": "news_analyst",
  "agent_name": "新闻舆情分析师",
  "status": "completed",
  "output": "分析结果...",
  "tokens": 1500,
  "thoughts": [...],
  "data_sources": [...]
}
```

---

### 历史记录查询

#### 查询股票历史
```http
GET /api/analysis/db/history/stock/600000?limit=10
```

**响应**：
```json
{
  "stock_code": "600000",
  "total": 5,
  "sessions": [
    {
      "session_id": "session_xxx",
      "stock_code": "600000",
      "status": "completed",
      "progress": 100,
      "created_at": "2025-12-08T06:30:00"
    }
  ]
}
```

#### 查询完整会话历史
```http
GET /api/analysis/db/history/session/{session_id}/full
```

**响应**：
```json
{
  "session": {
    "session_id": "session_xxx",
    "stock_code": "600000",
    "status": "completed",
    "progress": 100
  },
  "agent_results": [
    {
      "agent_id": "news_analyst",
      "status": "completed",
      "output": "...",
      "tokens": 1500,
      "duration_seconds": 15
    }
  ]
}
```

#### 查询最近分析
```http
GET /api/analysis/db/history/recent?limit=20
```

#### 查询热门股票
```http
GET /api/analysis/db/history/popular-stocks?limit=10
```

---

### 统计分析

#### 统计概览
```http
GET /api/analysis/db/stats/overview?days=7
```

**响应**：
```json
{
  "analysis": {
    "total_count": 50,
    "status_distribution": {
      "completed": 45,
      "error": 3,
      "running": 2
    },
    "avg_duration_seconds": 180,
    "period_days": 7
  },
  "agents": [
    {
      "agent_id": "news_analyst",
      "total_runs": 50,
      "avg_duration": 15,
      "max_duration": 30,
      "total_tokens": 75000
    }
  ]
}
```

#### 智能体统计
```http
GET /api/analysis/db/stats/agents?days=7
```

---

## 💻 前端集成

### 使用数据库 API

只需将前端的 API 端点从 `/api/analysis/session/` 改为 `/api/analysis/db/session/`：

```javascript
// 创建会话
const response = await fetch('http://localhost:8000/api/analysis/db/session/create', {
  method: 'POST',
  headers: {'Content-Type': 'application/json'},
  body: JSON.stringify({
    stock_code: '600000',
    stock_name: '浦发银行'
  })
})
```

### 查询历史记录

```javascript
// 查询某股票的历史
const history = await fetch(
  'http://localhost:8000/api/analysis/db/history/stock/600000'
).then(r => r.json())

console.log(`${history.stock_code} 被分析了 ${history.total} 次`)

// 查询热门股票
const popular = await fetch(
  'http://localhost:8000/api/analysis/db/history/popular-stocks'
).then(r => r.json())

console.log('热门股票:', popular.stocks)
```

---

## 🔧 配置

### 切换数据库

默认使用 SQLite，可以通过环境变量切换到 PostgreSQL：

```bash
# .env 文件
DATABASE_URL=postgresql://user:password@localhost/alphacouncil
```

### 数据库位置

- **SQLite**: `./alphacouncil.db`
- **PostgreSQL**: 配置的服务器地址

---

## 📈 使用场景

### 1. 查看历史分析

```python
# 查看某股票的所有历史分析
GET /api/analysis/db/history/stock/600000

# 查看某次分析的完整详情
GET /api/analysis/db/history/session/session_xxx/full
```

### 2. 性能分析

```python
# 查看智能体性能统计
GET /api/analysis/db/stats/agents?days=30

# 找出最慢的智能体
# 查看平均耗时、最大耗时
```

### 3. 热度分析

```python
# 查看最热门的股票
GET /api/analysis/db/history/popular-stocks

# 查看最近分析的股票
GET /api/analysis/db/history/recent
```

---

## 🎯 优势对比

| 功能 | 内存方案 | 数据库方案 |
|------|---------|-----------|
| **数据持久化** | ❌ 重启丢失 | ✅ 永久保存 |
| **历史记录** | ❌ 无 | ✅ 完整历史 |
| **跨设备同步** | ⚠️ 需Redis | ✅ 原生支持 |
| **统计分析** | ❌ 无 | ✅ 强大查询 |
| **数据审计** | ❌ 无 | ✅ 完整追溯 |
| **实现复杂度** | ⭐ 简单 | ⭐⭐ 中等 |

---

## ⚠️ 注意事项

### 1. 数据库文件

SQLite 数据库文件 `alphacouncil.db` 会自动创建在项目根目录。

### 2. 性能

- SQLite 适合开发和小规模使用
- 生产环境建议使用 PostgreSQL

### 3. 备份

定期备份数据库文件：

```bash
# SQLite 备份
cp alphacouncil.db alphacouncil_backup_$(date +%Y%m%d).db

# PostgreSQL 备份
pg_dump alphacouncil > backup.sql
```

---

## 🚀 下一步

1. ✅ 数据库模型已创建
2. ✅ API 已实现
3. ⏳ 集成到分析流程（需要修改分析代码）
4. ⏳ 前端历史记录页面

---

## 📝 示例查询

### SQL 查询示例

```sql
-- 查看所有会话
SELECT * FROM analysis_sessions ORDER BY created_at DESC LIMIT 10;

-- 查看某股票的分析次数
SELECT stock_code, COUNT(*) as count 
FROM analysis_sessions 
GROUP BY stock_code 
ORDER BY count DESC;

-- 查看智能体平均耗时
SELECT agent_id, AVG(duration_seconds) as avg_time
FROM agent_results
WHERE status = 'completed'
GROUP BY agent_id
ORDER BY avg_time DESC;

-- 查看成功率
SELECT 
  status,
  COUNT(*) as count,
  ROUND(COUNT(*) * 100.0 / SUM(COUNT(*)) OVER(), 2) as percentage
FROM analysis_sessions
GROUP BY status;
```

---

## ✅ 测试清单

- [ ] 初始化数据库
- [ ] 创建会话
- [ ] 查询会话状态
- [ ] 更新智能体结果
- [ ] 查询历史记录
- [ ] 查看统计信息
- [ ] 测试跨设备同步

---

现在你拥有了完整的数据库持久化方案！🎉
