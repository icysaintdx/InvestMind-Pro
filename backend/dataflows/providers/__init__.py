...
# TDX Provider - 仅使用 Native Provider（纯Python）
# HTTP Provider 已移除，不再需要 Docker 服务
...
def get_best_tdx_provider():
    ...
    Native Provider 使用 pytdx 直接连接通达信服务器，无需外部服务。
    HTTP Provider 已移除，简化架构。

    Returns:
        TDX Provider 实例，如果不可用则返回 None
    """
