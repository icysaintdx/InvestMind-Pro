...
def save_settings(settings: Dict[str, Any]) -> bool:
    try:
        ...
        safe_settings = {
            k: v for k, v in settings.items() if "Secret" not in k and "Key" not in k
        }
...
async def get_settings():
    try:
        ...
        settings["cninfoAccessKey"] = (
            os.getenv("CNINFO_ACCESS_KEY", "")[:8] + "***"
            if os.getenv("CNINFO_ACCESS_KEY")
            else ""
        )
        settings["cninfoAccessSecret"] = (
            "******" if os.getenv("CNINFO_ACCESS_SECRET") else ""
        )
...
async def get_services():
    try:
        ...
        try:
            ...
            services.append(
                {"name": "News Monitor", "status": "unknown", "statusText": "Unknown"}
            )
...
async def manual_cleanup():
    try:
        ...
        try:
            ...
            results["news"] = storage.cleanup_old_news(
                days=settings.get("newsRetentionDays", 30)
            )
        except Exception as e:
        ...
        try:
            ...
            with get_db_context() as db:
                cutoff = datetime.now() - timedelta(
                    days=settings.get("analysisRetentionDays", 90)
                )
                deleted = (
                    db.query(AnalysisSession)
                    .filter(AnalysisSession.created_at < cutoff)
                    .delete()
                )
...
async def get_trading_time_status():
    ...
    try:
        ...
        is_trading = is_weekday and (in_morning or in_afternoon)
        ...
        if status == "pre_market":
            next_open_time = now.replace(hour=9, minute=30, second=0, microsecond=0)
        elif status == "lunch_break":
            next_open_time = now.replace(hour=13, minute=0, second=0, microsecond=0)
        elif status in ["after_hours", "weekend"]:
            # 计算下一个交易日
            days_ahead = 1
            if weekday == 4:  # 周五
                days_ahead = 3
                ...
                days_ahead = 1
            next_open_time = (now + timedelta(days=days_ahead)).replace(
                hour=9, minute=30, second=0, microsecond=0
            )

        # 计算距离下次开盘的秒数
        time_to_next_open = None
        if next_open_time:
            time_to_next_open = int((next_open_time - now).total_seconds())
        ...
        return {
            ...
            "data": {
                ...
                "weekday": weekday,
                "nextOpenTime": next_open_time.strftime("%Y-%m-%d %H:%M:%S")
                if next_open_time
                else None,
                "timeToNextOpen": time_to_next_open,
                "refreshIntervals": {
                    "kline": {"trading": 30000, "nonTrading": 300000},
                    "portfolio": {"trading": 10000, "nonTrading": 60000},
                    "strategyPlan": {"trading": 15000, "nonTrading": 60000},
                    "marketData": {"trading": 5000, "nonTrading": 60000},
                },
            },
        }
    except Exception as e:
        logger.error(f"Get trading time status failed: {e}")
        raise HTTPException(status_code=500, detail=str(e))
