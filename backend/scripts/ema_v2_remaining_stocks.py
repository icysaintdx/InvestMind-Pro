...
# -*- coding: utf-8 -*-
"""
EMA V2 剩余股票回测 - 601012隆基绿能, 002415海康威视
"""
...
def generate_simple_report(all_results: List[Dict]) -> str:
    ...
    lines.append("|----------|----------|------|------|")
    for code, name, industry in REMAINING_STOCKS:
    ...
    if successful:
        ...
        lines.append("## 统计摘要")
        lines.append("")
        lines.append(f"- 成功回测: {len(successful)}/2 只股票")
...
def main():
    print("🚀 EMA V2 策略剩余股票回测")
    print(f"📈 测试股票: {len(REMAINING_STOCKS)} 只")
    ...
    report_file = RESULTS_DIR / "ema_v2_remaining_report.md"
    ...
    successful = [r for r in all_results if r.get("success")]
    print(f"📊 成功: {len(successful)}/2 只股票")
    if successful:
        returns = [r["best_by_return"]["total_return_pct"] for r in successful]
