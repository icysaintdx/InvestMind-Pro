...
def download_stock_data(stock_code: str, start_date: str = "20200101", end_date: str = "20241231") -> bool:
    """下载股票数据"""
    try:
        import akshare as ak
        
        print(f"  下载 {stock_code} 数据...", end=" ")
...
def main():
    ...
    for stock_code, stock_name, industry in EXTENDED_STOCKS:
        if download_stock_data(stock_code):
            downloaded.append(stock_code)
    
    print(f"\n✅ 成功下载 {len(downloaded)}/{len(EXTENDED_STOCKS)} 只股票数据")
    ...
    for code in original_codes:
        ...
        if result_file.exists():
            with open(result_file, 'r', encoding='utf-8') as f:
                original_results[code] = json.load(f)
    
    # 加载ema_v2_results.json中的详细数据
    try:
