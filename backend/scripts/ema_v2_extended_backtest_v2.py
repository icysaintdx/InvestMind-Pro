...
def get_stock_data_from_tdx(stock_code: str) -> Optional[pd.DataFrame]:
    ...
    try:
        # 尝试从tdx_cache目录读取
        tdx_dir = project_root / "backend/data/tdx_cache"
        
        # 查找匹配的文件
        for file in tdx_dir.glob(f"{stock_code}_*.csv"):
            df = pd.read_csv(file)
            if len(df) > 50:
                # 标准化列名
...
def download_stock_data_no_proxy(stock_code: str, start_date: str = "20200101", end_date: str = "20241231") -> bool:
    """下载股票数据 - 禁用代理"""
    import os
    # 清除代理设置
    ...
    try:
        ...
        print(f"  下载 {stock_code} 数据...", end=" ", flush=True)
        ...
        # 保存数据
        output_file = DATA_DIR / f"{stock_code}_{start_date}_{end_date}.csv"
...
def run_stock_backtests(stock_code: str, stock_name: str, industry: str) -> Dict[str, Any]:
    ...
    try:
        ...
        print(f"❌ 数据加载失败: {e}")
        return {"stock_code": stock_code, "stock_name": stock_name, "industry": industry, "success": False, "error": str(e)}
...
def main():
    ...
    for stock_code, stock_name, industry in EXTENDED_STOCKS:
        ...
        cache_file = DATA_DIR / f"{stock_code}_20200101_20241231.csv"
        if cache_file.exists():
            print(f"  {stock_code} 数据已存在，跳过下载")
            downloaded.append(stock_code)
            continue
            
        if download_stock_data_no_proxy(stock_code):
            downloaded.append(stock_code)
    
    print(f"\n✅ 成功获取 {len(downloaded)}/{len(EXTENDED_STOCKS)} 只股票数据")
    ...
    # 第二步：运行回测
    print(f"\n{'='*60}")
    print("📊 步骤2: 运行EMA V2回测")
    print(f"{'='*60}")
    
    all_results = []
    for stock_code, stock_name, industry in EXTENDED_STOCKS:
        if stock_code in downloaded:
