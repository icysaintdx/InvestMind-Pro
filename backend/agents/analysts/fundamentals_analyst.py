...
# 导入Google工具调用处理器
from backend.agents.utils.google_tool_handler import GoogleToolCallHandler
...
def _get_company_name_for_fundamentals(ticker: str, market_info: dict) -> str:
    ...
    try:
        if market_info['is_china']:
            ...
            # 港股：使用改进的港股工具
            try:
                from backend.dataflows.improved_hk_utils import get_hk_company_name_improved
...
def create_fundamentals_analyst(llm, toolkit):
    ...
    def fundamentals_analyst_node(state):
        ...
        system_message = (
            ...
            f"- GET http://localhost:8000/api/akshare/financial/{ticker}  # 三大财务报表"
            f"- GET http://localhost:8000/api/akshare/financial/{ticker}/summary  # 最新财务摘要\n"
