...
def log_result(interface_name: str, api_code: str, data_sources: str, 
               permission: str, status: str, count: int = 0, 
               sample_data: Any = None, error: str = ""):
    """记录测试结果"""
    test_results.append({
        "接口名称": interface_name,
        "接口代码": api_code,
    ...
    try:
        ...
        for src in sources:
            try:
                ...
                error_msg = str(e)
                if "权限" in error_msg or "permission" in error_msg.lower() or "抱歉" in error_msg:
                    log_result(
                        f"新闻快讯-{src}",
                        f"news(src={src})",
                        src,
    ...
    try:
        ...
        for date in [today, yesterday]:
            try:
                ...
                error_msg = str(e)
                if "权限" in error_msg or "permission" in error_msg.lower() or "抱歉" in error_msg or "积分" in error_msg:
                    log_result(
                        f"上证E互动-{date}",
                        f"irm_qa_sh(trade_date={date})",
                        "上交所e互动",
...
def generate_summary_report():
    ...
    print(f"Tushare Token: {TUSHARE_TOKEN[:8]}...{TUSHARE_TOKEN[-4:]}" if TUSHARE_TOKEN else "未配置")
    print("-"*80)
    ...
    report = {
        ...
        "Token": f"{TUSHARE_TOKEN[:8]}...{TUSHARE_TOKEN[-4:]}" if TUSHARE_TOKEN else "未配置",
        "统计": {
            "总接口数": total,
            "正常可用": normal,
    ...
    report_path = os.path.join(os.path.dirname(__file__), "tushare_news_interface_report.json")
