...
# API URLs
API_URLS = {
    "kimi": "https://kirocpa.zeabur.app/v1/chat/completions",
...
def get_api_key(provider: str) -> str:
    ...
    # kiminim和kimi都使用相同的API key（kirocpa中转）
    return os.getenv(env_keys.get(provider, ""), "") or ("icysaintdx" if provider in ["kimi", "minimax"] else "")
...
class LLMService:
    ...
    def _infer_provider_from_model(self, model: str) -> Optional[str]:
        ...
        if model_lower.startswith("minimax"):
            return "kimi"  # minimax也走kimi通道（同一个中转）

        # 包含斜杠的模型名（如 Qwen/Qwen3-8B）走kirocpa中转（SiliconFlow余额为0）
        if "/" in model:
            return "kimi"  # 走kimi通道（同一个中转）
    ...
    async def call_llm(
        ...
        if task_name:
            ...
            provider = provider or self._infer_provider_from_model(model) or config.get("provider", "kimi")
        else:
        ...
        if not api_key:
            ...
            # 尝试备用提供商
            for backup_provider in ["kimi", "minimax", "deepseek", "qwen", "siliconflow"]:
