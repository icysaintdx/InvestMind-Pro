...
# -*- coding: utf-8 -*-
"""
EMA V2策略专用回测 - 使用batch_backtest_all相同机制
"""
...
def run_backtest_for_stock(strategy, stock_code: str, data: pd.DataFrame):
    """为单只股票运行回测"""
...
def main():
    ...
    for stock_code, stock_name in STOCKS.items():
        ...
        config = StrategyConfig(
            name="ema_breakout_v2",
            parameters={}
        )
        strategy = EMABreakoutV2Strategy(config)
        
        result = run_backtest_for_stock(strategy, stock_code, data)
        if result:
